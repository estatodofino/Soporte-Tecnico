...
class Recovery_model extends CI_Model
{
	...
	*/
	...
	{
		$query = $this->db->get_where('usuario', array('correo_usuario' => $email));
        if($query->num_rows() === 1)
        {
            return TRUE;
        }
	...
	*/
	public function getUserData($email)
	{
        ...
        if($query->num_rows() === 1)
        {
        	...
        	if($this->startRecoveryPassword($query->row()->ci_usuario))
        	{
        		return $query->row();
        	}
        }
	...
	*/
	private function startRecoveryPassword($user_id)
	{
		...
		$this->db->where("ci_usuario", $user_id);
	...
	*/
	public function checkIsLiveToken($token)
	{
		...
		$query = $this->db->select("ci_usuario")
				 ->from("usuario")
				 ->where("token",$token)
