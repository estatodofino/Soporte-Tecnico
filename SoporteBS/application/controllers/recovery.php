...
class Recovery extends CI_Controller
{
    ...
    */
    ...
    {
        $this->load->view("overall/header2", $data);
        $this->load->view($view, $data);
        $this->load->view("overall/footer2", $data);
    }
    ...
    */
    public function recovery_password($token)
    {
        ...
        /*$data = array();
        $data["token"] = $token;*/
        $id = $this->checkIsLiveToken($token);
        $this->session->set_userdata("id_user_recovery_pass",$id->ci_usuario);//$data["token"] = $token;
        $data = array('token' =>$token,
        'usuario'=>$id->ci_usuario );
    ...
    */
    public function update_password()
    {
        ...
        else
        {
            ...
            if($this->recovery_model->change_password($id,$data) === TRUE)
            {
                $this->session->set_flashdata(
    ...
    */
    private function sendMailRecoveryPass($userdata)
    {
        ...
        $configGmail = array(
            ...
            'smtp_port' => 465,
            'smtp_user' => 'soporte.luzpf@gmail.com',
            'smtp_pass' => 'puroparo',
            'mailtype' => 'html',
            'charset' => 'utf-8',
            'newline' => "\r\n"
        ...
        $this->email->from('soporte.luzpf@gmail.com');
        $this->email->to($userdata->correo_usuario);
        ...
        $html .= '<a href="http://localhost/SoporteBS/recovery/recovery_password/'.$userdata->token.'">';
        $html .= 'http://localhost/SoporteBS/recovery/recovery_password/'.$userdata->token.'</a>';
