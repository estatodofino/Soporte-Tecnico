...
class Componentes extends CI_Controller {
   ...
   public function store()
   {
    ...
    if ($this->form_validation->run()) {
      ...
      if ($this->Componentes_model->save_partes($data)) {
        ...
        redirect(base_url()."admin/Componentes/add".$codigo_equipo);
      }
    }
    else{
 ...
 public function update()
 {
  ...
  protected function save_compo_equipo($codigo_equipo,$serial,$tipo)
   ...
   public function new_sof()
   {
    $codigo_equipo = $this->input->post("numero_so");//numero del bien
    $tipo = $this->input->post("tipo_so"); //codigo tipo de equipo
    $codigo = "3";//nombre del componente
    $serial = $this->input->post("serial_so");//serial del componente
    $descripcion = $this->input->post("descrip_so");
    $marca="100";
   ...
   public function editar_so($id)
    {
      if ($this->session->userdata("rol")==3)
      {
       ...
       $this->load->view('overall/header2');
       $this->load->view('overall/menuAdmin');
       $this->load->view('admin/componentes/editar_so', $data);
       $this->load->view('overall/footer2');
     }
     elseif ($this->session->userdata("rol")==2)
      {
    ...
    {
       $serial=$this->input->post("serialiso");
       $descripcion = $this->input->post("descripcion");
       $condicion = $this->input->post("estado");

       $this->form_validation->set_rules("descripcion","Descripcion","required");
       $this->form_validation->set_rules("estado","Condicion","required");
       ...
       if ($this->form_validation->run()) {
        ...
        if ($this->Componentes_model->updateCompo($serial,$data)) {
         $this->session->set_flashdata("success","Se ha guardado la informacion");
          ...
          $this->session->set_flashdata("error","No se pudo guardar la informacion");
           redirect(base_url()."admin/Componentes/editar_so".$serial);
         }
       }else{
         $this->editar_so($serial);
       }
