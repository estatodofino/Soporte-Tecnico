...
function mostrarDatos(valor){
	$.ajax({
		url:"http://localhost/Appurbi/empleados/mostrar",
		type:"POST",
		data:{buscar:valor},
		success:function(respuesta){
			//alert(respuesta);
			...
			html ="<table class='table table-responsive table-bordered'><thead>";
 			html +="<tr><th>#</th><th>Nombres</th><th>Apellidos</th><th>DNI</th><th>Telefono</th><th>Email</th><th>Imagen</th><th>Usuario</th><th>Accion</th></tr>";
			html +="</thead><tbody>";
			for (var i = 0; i < registros.length; i++) {
				html +="<tr><td>"+registros[i]["id_empleado"]+"</td><td>"+registros[i]["nombres_empleado"]+"</td><td>"+registros[i]["apellidos_empleado"]+"</td><td>"+registros[i]["dni_empleado"]+"</td><td>"+registros[i]["telefono_empleado"]+"</td><td>"+registros[i]["email_empleado"]+"</td><td><img src='http://localhost/Appurbi/assets/images/uploads/"+registros[i]['foto_empleado']+"' style='width:100px; height:100px;'/></td><td>"+registros[i]["nombres"]+"</td><td><a href='"+registros[i]["id_empleado"]+"' class='btn btn-warning' data-toggle='modal' data-target='.bs-example-modal-lg'>E</a> <button class='btn btn-danger' type='button' value='"+registros[i]["id_empleado"]+"'>X</button></td></tr>";
			};
			html +="</tbody></table>";
...
function actualizar(){
	...
	$.ajax({
		url:"http://localhost/Appurbi/empleados/actualizar",
		type:"POST",
		data:formData,
		cache:false,
		processData:false,
...
function eliminar(idsele){
	$.ajax({
		url:"http://localhost/Appurbi/empleados/eliminar",
		type:"POST",
		data:{id:idsele},
		success:function(respuesta){
			alert(respuesta);
